  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {activeTab === 'DIGITAL' ? (
            <>
                <StatCard title="Digital Vouchers Claimed" value={`${stats.claimedDigitalVouchers} / ${stats.totalDigitalVouchers}`} icon={<Users />} />
                <StatCard title="Redeemed" value={vouchers.filter(v => v.type === 'DIGITAL' && v.isRedeemed).length} icon={<TicketCheck />} />
                <StatCard title="Claims Today" value={stats.claimsToday} icon={<CalendarClock />} />
                <StatCard title="Reimbursement" value={`Rp ${(vouchers.filter(v=>v.type==='DIGITAL' && v.isRedeemed).length * 10000).toLocaleString('id-ID')}`} icon={<span className="font-bold">Rp</span>} />
            </>
        ) : (
             <>
                <StatCard title="Physical Vouchers Used" value={`${stats.redeemedPhysicalVouchers} / ${stats.totalPhysicalVouchers}`} icon={<Warehouse />} />
                <StatCard title="Total Redeemed" value={stats.redeemedPhysicalVouchers} icon={<TicketCheck />} />
                 <StatCard title="Used Today" value={vouchers.filter(v => v.type === 'PHYSICAL' && v.claimDate.startsWith(new Date().toISOString().split('T')[0])).length} icon={<CalendarClock />} />
                <StatCard title="Reimbursement" value={`Rp ${(stats.redeemedPhysicalVouchers * 10000).toLocaleString('id-ID')}`} icon={<span className="font-bold">Rp</span>} />
            </>
        )}
      </div>
      ...
      {activeTab === 'DIGITAL' && (
         <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <div className="bg-white p-6 rounded-xl shadow-lg">
                <h3 className="text-lg font-semibold mb-4">Claims per Outlet</h3>
                <ResponsiveContainer width="100%" height={300}>
                    <BarChart data={OUTLETS.map(o => ({name: o, claims: stats.claimsByOutlet?.[o] || 0}))}>
                        <CartesianGrid strokeDasharray="3 3" /><XAxis dataKey="name" tick={false} /><YAxis /><Tooltip /><Legend /><Bar dataKey="claims" fill="#ea580c" />
                    </BarChart>
                </ResponsiveContainer>
            </div>
            <div className="bg-white p-6 rounded-xl shadow-lg">
                <h3 className="text-lg font-semibold mb-4">Claims per Day</h3>
                <ResponsiveContainer width="100%" height={300}>
                    <BarChart data={stats.claimsPerDay.slice(-30)}>
                        ...
                        <YAxis /><Tooltip /><Legend /><Bar dataKey="count" name="claims" fill="#f97316" />
                    </BarChart>
                </ResponsiveContainer>
            </div>
        </div>
      ...
      <div className="bg-white p-6 rounded-xl shadow-lg">
        <div className="flex flex-col sm:flex-row justify-between items-center mb-4 gap-4">
          <h2 className="text-xl font-bold">{activeTab === 'DIGITAL' ? 'Digital' : 'Physical'} Voucher Data</h2>
          <div className="flex items-center gap-4 w-full sm:w-auto">
             <div className="w-full sm:w-56">
                <SearchableSelect
                    id="outlet-filter" label="" options={['All Outlets', ...OUTLETS]} value={filterOutlet}
                    onChange={(value) => { setFilterOutlet(value); setCurrentPage(1); }}
          ...
          <table className="w-full text-sm text-left text-gray-500">
            <thead className="text-xs text-gray-700 uppercase bg-gray-50">
              <tr>
                ...
                <th scope="col" className="px-6 py-3">Voucher Code</th>
                <th scope="col" className="px-6 py-3">Outlet</th>
                <th scope="col" className="px-6 py-3">{activeTab === 'DIGITAL' ? 'Claim Date' : 'Redeemed Date'}</th>
                <th scope="col" className="px-6 py-3">Status</th>
                {activeTab === 'PHYSICAL' && <th scope="col" className="px-6 py-3">Notes</th>}
              </tr>
            </thead>
            <tbody>
        ...
        <div className="flex justify-between items-center mt-4">
            <span className="text-sm text-gray-700">
                Showing {Math.min((currentPage - 1) * VOUCHERS_PER_PAGE + 1, filteredVouchers.length)} to {Math.min(currentPage * VOUCHERS_PER_PAGE, filteredVouchers.length)} of {filteredVouchers.length}
            </span>
            <div className="flex gap-2">
                <Button onClick={() => setCurrentPage(p => Math.max(1, p - 1))} disabled={currentPage === 1} size="sm">Previous</Button>
      ...
      <div className="bg-red-50 p-6 rounded-xl shadow-lg border border-red-200 mt-8">
        ...
        <p className="text-sm text-red-600 mt-1 mb-4">
          This action is irreversible. It will permanently delete all claimed vouchers, redeemed vouchers, and uploaded codes. The dashboard will be reset to zero.
        </p>
        <Button 
          onClick={resetData} 
          variant="primary"
